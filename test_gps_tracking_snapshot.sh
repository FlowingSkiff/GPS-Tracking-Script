...
coor=$(cat $test_file | grep -m 1 TPV | jq -r '[.lat, .lon] | @csv')
echo Coordinates: $coor
echo " "

alt=$(cat $test_file | grep -m 2 TPV | jq -r '[.altHAE] | @csv')
echo Altitude\(M\): $alt
echo " "
...
for sv in $(cat $test_file |  grep -oP '\{"PRN":.*?\}' | grep -oP '\{"PRN":\b(13|22|21|11|20|18|16|19|2|1|5|31|17|15|29|12|1|3|6|8|9|10|24|25|26|27|30|32|4|18|23|14|11|28)\b.*?\}' | sort -u -t, -k1,1 | awk -F, '{print $1}' | sed 's/{"PRN"://' )
do
	echo ${SV_Array[$sv]}
done
echo " "
